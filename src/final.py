# === Combined script generated from temp5_1.py and temp7_550_1.py ===
# File: temp5_1.py
...
# --------------------------------------------------
# 1. build similarity matrix with any sim function
# --------------------------------------------------
def build_sim_matrix(docs, sim_func, **kwargs):
    n = len(docs)
    ...
    for i in range(n):
        for j in range(i, n):
            if i == j:
                ...
                s = sim_func(docs[i][1], docs[j][1], **kwargs)
                M[i, j] = M[j, i] = s
    return M

# --------------------------------------------------
# 2. end-to-end latency experiment
# --------------------------------------------------
def latency_experiment(json_docs, n_clusters=3,
                       sim_func=mcjsim, sim_kw=None,
                       alt_func=jaccard_similarity_exact):
    if sim_kw is None:
        sim_kw = {}
        
    # --- proposed similarity ---------------------------------
    with Timer() as tic_sim:
        S = build_sim_matrix(json_docs, sim_func, **sim_kw)
    with Timer() as tic_cluster:
        D = 1 - S
        model = AgglomerativeClustering(n_clusters=n_clusters,
                                        metric='precomputed',
                                        linkage='average')
        labels = model.fit_predict(D)
    total_proposed = tic_sim.elapsed + tic_cluster.elapsed
    
    # --- baseline exact-Jaccard ------------------------------
    with Timer() as tic_baseline:
        J = build_sim_matrix(json_docs,                  # doc list
                             lambda a,b: alt_func(fingerprint(a),
                                                  fingerprint(b)))
    total_baseline = tic_baseline.elapsed   # no clustering here

    print(f"\n--- Latency report ({len(json_docs)} documents) ---")
    print(f"Similarity matrix (MC-JSim) : {tic_sim.elapsed:.3f} s")
    print(f"Clustering step            : {tic_cluster.elapsed:.3f} s")
    print(f"TOTAL (proposed)           : {total_proposed:.3f} s")
    print(f"TOTAL (exact Jaccard only) : {total_baseline:.3f} s")
    return labels, S
...
for label, a, b in experiments:
    ...
    mh_a = minhash(fp_a, 128)
    mh_b = minhash(fp_b, 128)
    jaccard_minhash_val = jaccard_minhash(mh_a, mh_b)

    sim = mcjsim(a, b, 128, alpha=0.4, beta=0.4) # Adjust weights as needed
...
def perform_json_clustering(
    ...
    Args:
        ...
        sim_func: The similarity function to use (e.g., mcjsim).
...
perform_json_clustering(
    cluster_docs, 
    n_clusters=3,
    sim_func_kwargs={"alpha": 0.4, "beta": 0.4} # Pass weights to mcjsim
)
print("\n" + "="*50 + "\n")

# labels, sim_mat = latency_experiment(
#     json_docs=cluster_docs,        # your list of (name, doc)
#     n_clusters=3,
#     sim_func=mcjsim,                  # ← any similarity you want to test
#     sim_kw=dict(num_perm=128, alpha=0.4, beta=0.4)
# )
...
for label, a, b in experiments:
    ...
    mh_a = minhash(fp_a, 64)
    mh_b = minhash(fp_b, 64)
    jaccard_minhash_val = jaccard_minhash(mh_a, mh_b)
...
# ----------------------------------------------------------------------
# E3. Latency & throughput benchmark
# ----------------------------------------------------------------------
import gc, time, statistics as st, pandas as pd
from typing import List, Tuple

def _make_flat_json(n: int) -> dict:
    """Synthetic object with ~n primitive nodes."""
    return {f"k{i}": i for i in range(n)}

def throughput_table(
    sizes: List[int] = (100, 1_000, 10_000),
    trials: int = 5,
    num_perm: int = 64,
    alpha: float = 0.4,
    beta:  float = 0.4
    ...
    """
    Measure fingerprint, sketch, and comparison latency for each size in `sizes`.
    Returns a DataFrame with µs/node (fp & sketch) and µs/pair (cmp).
    """
    ...
    for n in sizes:
        ...
        for _ in range(trials):
            ...
            fpA = fingerprint(A)
            fpB = fingerprint(B)
            fp_t.append(time.perf_counter() - t0)
            gc.enable()

            # --- sketch construction -----------------------------------
            gc.disable()
            t1 = time.perf_counter()
            mhA = minhash(fpA, num_perm=num_perm)
            mhB = minhash(fpB, num_perm=num_perm)
            sk_t.append(time.perf_counter() - t1)
            gc.enable()

            # --- similarity --------------------------------------------
            gc.disable()
            t2 = time.perf_counter()
            _   = mcjsim(A, B, num_perm=num_perm, alpha=alpha, beta=beta)
            cmp_t.append(time.perf_counter() - t2)
            gc.enable()

        rows.append((
            n,
            st.mean(fp_t)  / n * 1e6,           # µs / node
            st.mean(sk_t)  / n * 1e6,           # µs / node
            st.mean(cmp_t) * 1e6                # µs / pair
        ))

    df = pd.DataFrame(
        rows,
        columns=["nodes",
                 "fingerprint_µs_per_node",
                 "sketch_µs_per_node",
                 "compare_µs_per_pair"]
    )
    return df

tbl = throughput_table(sizes=[100, 1_000, 10_000], trials=3)
print(tbl.to_string(index=False, formatters={
    "fingerprint_µs_per_node": "{:8.2f}".format,
    "sketch_µs_per_node"     : "{:8.2f}".format,
    "compare_µs_per_pair"    : "{:8.2f}".format
}))
...
#     _ = minhash(fp, 64) # Simulate generating one sketch
...
# --- E5. Ablation study (code implementation) -------------------------------
from sklearn.metrics import roc_auc_score

labels = []            # 1 = identical, 0 = edited
sim_full = []          # α = 0.4, β = 0.4, γ = 0.2
sim_no_content = []    # β = 0   (content removed)
sim_no_struct   = []   # γ = 0   (struct-type removed)

for name, doc_a, doc_b in experiments:
    labels.append(1 if "identical" in name else 0)

    sim_full.append(mcjsim(doc_a, doc_b, alpha=0.4, beta=0.4, num_perm=64))
    sim_no_content.append(mcjsim(doc_a, doc_b, alpha=0.6, beta=0.0, num_perm=64))
    sim_no_struct.append(mcjsim(doc_a, doc_b, alpha=0.6, beta=0.4, num_perm=64))

print("\n--- E5. ROC-AUC comparison ---")
print(f"Full model         : {roc_auc_score(labels, sim_full):.3f}")
print(f"No content term    : {roc_auc_score(labels, sim_no_content):.3f}")
print(f"No structural term : {roc_auc_score(labels, sim_no_struct):.3f}")

# --- End of temp7_550_1.py ---

# labels, sim_mat = latency_experiment(
#     json_docs=e4_cluster_docs,        # your list of (name, doc)
#     n_clusters=3,
#     sim_func=mcjsim,                  # ← any similarity you want to test
#     sim_kw=dict(num_perm=64, alpha=0.4, beta=0.4)
# )